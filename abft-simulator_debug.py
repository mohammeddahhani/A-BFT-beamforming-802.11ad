...
parser.add_argument("--debug", help="Per period logs",action="store_true")
parser.add_argument("--tofile", help="Write result to",action="store_true")
...
"""
	...
	- idle_log: the last period the station was idle (0 if never been idle)
"""
...
def start_period(pid, slot):
	...
	if log: print("[-] first slot (tx:{})".format(tx))
	if tx == 0: # all active stations in previous period succeeded their attempts
		for sta in xrange(STA):
			if stations[sta].retx_pid == 0 and stations[sta].is_idle == 0:
				...
				pid = new_pkt.id
				if log: 
					print("[+] pid:{0} (sta={4},retx={3}) - BO-new: {1} -> {2}"\
						.format(pid, boff, boff+slot, new_pkt.retx_count, new_pkt.sta))
		...
		for pkt in timeslots[slot]: 
			...
			_active_stas +=1	
			if log: 
				print("[+] pid:{0} (sta={4},retx={3}) - BO-old: {1} -> {2}"\
						.format(pkt.id, boff, boff+slot, pkt.retx_count, pkt.sta))
		...
		if log: 
			print('[i] {} colliding in current slot'.format(len(first_slot_tx)))
...
def handle_slot(slot, contending, first_slot=False):
	if log and  first_slot: print("[i] first slot")
	...
	if tx > 1: # number of contending stations
		if log:	print("[+] collisions {}".format(tx))
		log_coll[slot] += 1
		log_tx[slot]   += tx
		log_retx[slot] += tx
		...
		for pkt in contending:
			...
			if log and pkt.retx_count < MAX_ATTEMPT:
				print("[+] pid:{0} (sta={6},retx={5}) - BO-middle: {1} -> {2} in [{3} - {4}]"\
					.format(pkt.id, boff, boff+slot+1, start, end, pkt.retx_count, sid))			
			
			if pkt.retx_count < MAX_ATTEMPT:
				if slot+boff+1 > end: 
					if log: print('[-] -- OUTside period boundaries')
					(start, end) = get_period_boundaries(slot+boff+1)
				else:
					if log: print('[+] -- INside period boundaries')
					start = slot+1+boff
			...
			# Sta becomes IDLE
				...
				if log: 
					print("[+] pid:{0} sta:{1} MAX attempts -> move to {2} (BO:{3}) "\
						.format(pkt.id, pkt.sta, start, boff))
				if log:
					print '[++] sta:{} pkt:{} oper:{} -> {} ({}, {})'\
		...
		if log: 
			_pid, _sta, _pslot, _retx, _rlog = contending[0]
			print ("[i] pkt:{} sid:{} retx:{} slot:{} rlog:{}")\
						.format(_pid, _sta, _retx, _pslot, _rlog)
			print("[+] tx {0} - sta {1}".format (_pid,_sta))
		"""
			Logging stats
		"""
		...
		if log: 
			print("[+] succ after {} slots".format(slots))
			print("[+] succ after {} periods".format(periods))
...
while s_index < MAX_SLOTS:
	...
	if s_index % NS == 0 and s_index > 0 and log:
		print("#####################-----------------#########################")	
	if log:	
		print("\nSLOT {0} - PERIOD {1}".format(s_index, period))
	...
	if s_index > 0 and s_index % NS == 0 : # begining of period and not first period
		...
		if log: 
			print("[s] {} actives - period {}".format(log_active_sta[period-1], period-1))
			print "active count:",log_active_sta[:-1]
			print

		cond_idle_count = log_max_retx[period-1]
		per_period_cond_idle_count.append(cond_idle_count)
		if log: 
			print("[s] {} to 'idle' - period {}".format(idle_count,period-1))
			print "idle count:",per_period_cond_idle_count
			print

		success= np.sum(log_th[(s_index-NS):s_index])
		per_period_success.append(success)
		if log: 
			print("[s] {} success - period {} ([{},{}])".format(success, period-1, s_index-NS, s_index-1))
			print("success count:",per_period_success)
			print
		
		coll= np.sum(log_coll[(s_index-NS):s_index])
		if log: 
			print("[s] {} collisions - period {} ([{},{}])".format(coll, period-1, s_index-NS, s_index-1))
			print "collisions count:",log_coll[(s_index-NS):s_index]
			print
		
		tx= np.sum(log_tx[(s_index-NS):s_index])
		if log: 
			print("[s] {} traffic - period {} ([{},{}])".format(tx, period-1, s_index-NS, s_index-1)) 
			print "traffic count:",log_tx[(s_index-NS):s_index]
			print

		empty= np.sum(log_idle[(s_index-NS):s_index])
		if log: 
			print("[s] {} empty slots - period {} ([{},{}])".format(empty, period-1, s_index-NS, s_index-1))
			print "empty count:",log_idle[(s_index-NS):s_index]
			print
...
#print('[+] Proba of Idle:            {0:.3f}% (old:{1:.3f}%)'\
#					.format(idle_proba*100, cond_idle_proba*100))
#print('[+] Proba of success:         {0:.3f} (old:{1:.3f})'\
#					.format(succ_proba, old_succ_proba))
